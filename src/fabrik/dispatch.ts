...
export const dispatchRun = (options: DispatchOptions): DispatchResult => {
  ...
  const smithersScript = [
    ...
    "export PATH=\"$HOME/.bun/bin:$PATH\"",
    `# Install smithers locally to ensure correct module resolution for JSX`,
    `if [ ! -f "${controlDir}/node_modules/.bin/smithers" ]; then`,
    `  echo "[${options.vm}] Installing smithers locally..."`,
    `  bun init -y 2>/dev/null || true`,
    `  bun add smithers-orchestrator@github:evmts/smithers#ea5ece3b156ebd32990ec9c528f9435c601a0403 zod 2>&1 || true`,
    `fi`,
    `export PATH="${controlDir}/node_modules/.bin:$PATH"`,
    "if [ -f ~/.config/ralph/ralph.env ]; then set -a; source ~/.config/ralph/ralph.env; set +a; fi",
