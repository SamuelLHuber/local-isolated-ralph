...
export const dispatchRun = (options: DispatchOptions): DispatchResult => {
  ...
  const smithersScript = [
    ...
    `  echo "[${options.vm}] Installing smithers-orchestrator for JSX resolution..."`,
    `  cd "${projectRootInVm}" && bun add -d smithers-orchestrator@github:evmts/smithers#ea5ece3b156ebd32990ec9c528f9435c601a0403 2>&1 || true`,
    `fi`,
    `cd "${controlDir}"`,
    "if [ -f ~/.config/ralph/ralph.env ]; then set -a; source ~/.config/ralph/ralph.env; set +a; fi",
