...
export const dispatchRun = (options: DispatchOptions): DispatchResult => {
  ...
  const smithersScript = [
    ...
    `fi`,
    `# Ensure smithers-orchestrator is in project for JSX runtime resolution`,
    `if [ ! -d "${projectRootInVm}/node_modules/smithers-orchestrator" ]; then`,
    `  echo "[${options.vm}] Installing smithers-orchestrator with React 19..."`,
    `  cd "${projectRootInVm}" && bun add -d smithers-orchestrator@github:evmts/smithers#ea5ece3b156ebd32990ec9c528f9435c601a0403 react@^19.2.4 react-dom@^19.2.4 2>&1 || true`,
    `fi`,
    `cd "${controlDir}"`,
    "if [ -f ~/.config/ralph/ralph.env ]; then set -a; source ~/.config/ralph/ralph.env; set +a; fi",
