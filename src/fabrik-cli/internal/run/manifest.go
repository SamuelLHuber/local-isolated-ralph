...
func writePodTemplate(b *strings.Builder, indent string, opts Options, sharedBundle sharedCredentialBundle, labels, annotations map[string]string, workflowSecretName, syncSecretName, pvcName string) {
	...
	if strings.TrimSpace(opts.WorkflowPath) == "" {
		...
		// Install Playwright browsers before the workflow starts (requires network).
		// Uses bun from PATH to run the project-local playwright, falling back to
		// the runtime copy. This is idempotent — a second install is a fast no-op.
		bootstrap += " && if command -v bun >/dev/null 2>&1; then echo '[fabrik] installing Playwright browsers...' && bun x playwright install --with-deps chromium 2>&1 || echo '[fabrik] Playwright browser install failed (non-fatal)'; fi"
