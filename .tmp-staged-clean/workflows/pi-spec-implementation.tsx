...
const WORKSPACES_DIR = resolve(REPO_ROOT, "workflows", ".jj-workspaces");
const DB_PATH = resolve(REPO_ROOT, "workflows", "pi-spec-implementation.db");
...
const FEATURE_BOOKMARK = "feat/rectum-demo";
...
const MIN_REVIEWER_IDS = new Set(["baseline", "security"]);
...
function readSpecsBundle(): string {
  const specsDir = resolve(REPO_ROOT, "specs");
  const files = listMarkdownFiles(specsDir);
  const contents = files.map((file) => {
    const name = file.replace(`${specsDir}/`, "specs/");
    return `\n---\n# ${name}\n\n${readText(file)}\n`;
  });
  return contents.join("\n");
}

function readReviewerPrompts(): Reviewer[] {
  const reviewersDir = resolve(REPO_ROOT, "prompts", "reviewers");
  const files = listMarkdownFiles(reviewersDir);
  return files.map((file) => {
    ...
    return {
      id,
      title,
      prompt: readText(file),
    };
  });
}

function filterReviewers(reviewers: Reviewer[], ids: Set<string>): Reviewer[] {
  return reviewers.filter((reviewer) => ids.has(reviewer.id));
}

const VISION = readText(resolve(REPO_ROOT, "VISION.md"));
const ROOT_AGENT = readText(
  firstExistingPath(resolve(REPO_ROOT, "AGENTS.md"), resolve(REPO_ROOT, "CLAUDE.md")),
);
const API_AGENT = readText(resolve(REPO_ROOT, "apps", "api", "AGENTS.md"));
const SPECS_BUNDLE = readSpecsBundle();
const REVIEWERS = readReviewerPrompts();
const MIN_REVIEWERS = filterReviewers(REVIEWERS, MIN_REVIEWER_IDS);
...
const SYSTEM_PROMPT = [
  "You are the PI implementation agent for Tectum.",
  "Follow all instructions exactly. Use minimal dependencies and German precision.",
  "",
  "Vision:",
  VISION,
  "",
...
Return ONLY JSON matching the schema.`}
      ...
      <Task
        ...
        output={outputs.report}
        timeoutMs={TASK_TIMEOUT_MS}
        skipIf={!allReviewersApproved(ctx, ticketId, REVIEWERS, "final-review")}
      >
      ...
      <Sequence>
        <Branch if={pending.length === 0} then={
          <Task
            ...
            retries={2}
          >
            {`Discover the next 3-5 implementation tickets by comparing all specs to the current codebase.

Constraints:
- Start by reading specs/README.md.
